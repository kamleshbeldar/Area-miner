...
CSV_FIELDS = [
    ...
    "phone", "phone_confidence", "email", "email_confidence", "email_maps",
    "website", "website_status", "social_profiles", "social_profiles_maps",
    "whatsapp", "contact_page", "technologies",
    "address", "business_hours", "price_range", "plus_code",
...
def _is_session_dead(error: str) -> bool:
    """Return True when the error indicates the Playwright browser context is gone."""
    dead_phrases = (
    ...
    lower = error.lower()
    return any(phrase in lower for phrase in dead_phrases)


def _restart_session(job_id: str, worker_number: int, config: AreaConfig,
                     old_session) -> object | None:
    """Close the dead session and open a fresh one. Returns new session or None on failure."""
    try:
        if old_session:
            old_session.close()
    except Exception:
    ...
    for attempt in range(1, 4):
        try:
            log(job_id, f"W{worker_number} restarting browser session (attempt {attempt})…")
            new_session = open_worker_session(config)
            log(job_id, f"W{worker_number} browser session restarted successfully.")
            return new_session
        except Exception as exc:
            log(job_id, f"W{worker_number} session restart attempt {attempt} failed: {str(exc)[:120]}")
            time.sleep(2 * attempt)
    log(job_id, f"W{worker_number} could not recover browser session — worker exiting.")
    return None
        ...
        while not stop_event.is_set() and store.job_status(job_id) == "running":
            ...
            if stage == "discovery":
                ...
                try:
                    ...
                    log(job_id, f"W{worker_number} discovery {task['id']} failed: {err_msg[:180]}")
                    if _is_session_dead(err_msg):
                        session = _restart_session(job_id, worker_number, config, session)
                        if session is None:
                            break
            elif stage == "details":
                ...
                try:
                    ...
                    log(job_id, f"W{worker_number} detail {place['id']} failed: {err_msg[:180]}")
                    if _is_session_dead(err_msg):
                        session = _restart_session(job_id, worker_number, config, session)
                        if session is None:
                            break
...
def _geometry_size(geometry: dict) -> dict:
    """Return human-readable size info for the estimate response."""
    if geometry.get("type") == "circle":
        return {"radius_m": float(geometry.get("radius_m", 0))}
    try:
        from area_engine import polygon_points
        pts = polygon_points(geometry)
        if len(pts) >= 3:
            lats, lons = zip(*pts)
            lat_span = (max(lats) - min(lats)) * 110.574
            lon_span = (max(lons) - min(lons)) * 111.320 * math.cos(math.radians(sum(lats) / len(lats)))
            return {"area_km2": round(lat_span * lon_span, 3)}
    except Exception:
        pass
    return {}


def _validate_geometry_size(geometry: dict) -> None:
    """Raise ValueError if the drawn area is too small to be useful."""
    if geometry.get("type") == "circle":
        radius_m = float(geometry.get("radius_m", 0))
        if radius_m < 200:
            raise ValueError(
                f"Circle radius is only {radius_m:.0f} m — too small to capture any businesses. "
                "Draw a larger area (at least 200 m radius)."
            )
...
def estimate() -> Response:
    try:
        ...
        size_info = _geometry_size(geometry)
        return jsonify({"grid_points": len(points), "categories": len(categories), "queries": min(config.max_queries, len(points) * len(categories)), **size_info})
    except Exception as exc:
...
def start() -> Response:
    try:
        geometry = json.loads(request.form.get("geometry", "{}"))
        _validate_geometry_size(geometry)
