...
def detail_place(session: DynamicSession, config: AreaConfig, place: dict,
    ...
    if not point_in_area((float(candidate["latitude"]), float(candidate["longitude"])), config.geometry):
        return {"status": "skipped", "lead": None, "error": "outside selected boundary"}
