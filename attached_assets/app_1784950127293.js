...
if (form) {
  ...
  function serialize() {
    ...
    let geometry;
    if (selectedLayer instanceof L.Circle) geometry={type:'circle',center:[selectedLayer.getLatLng().lat,selectedLayer.getLatLng().lng],radius_m:selectedLayer.getRadius()};
    else geometry=selectedLayer.toGeoJSON().geometry;
  ...
  document.querySelector('#find-place').addEventListener('click', async () => {
    ...
    if(!place) return alert('City ya PIN code enter karein.');
  ...
  async function estimate() {
    ...
    document.querySelector('#estimate').innerHTML=response.ok?`<small>AREA PLAN</small><strong>${data.grid_points} grid points x ${data.categories} categories</strong><span>${data.queries} capped Google Maps queries</span>`:`<small>AREA ERROR</small><strong>${esc(data.error)}</strong>`;
  }
  ...
  form.addEventListener('submit',event => { if(!document.querySelector('#geometry').value){event.preventDefault();alert('Pehle map par circle, rectangle ya polygon select karein.');} });
}
